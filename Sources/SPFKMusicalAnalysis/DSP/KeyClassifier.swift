...
/// Classifies a 12-element pitch chroma vector into one of 24 musical keys by Pearson correlation
/// against all 24 cyclically-rotated Temperley key profiles, which perform well on recorded music.
///
/// Key index mapping:
...
struct KeyClassifier: Sendable {
    ...
    struct Result: Sendable {
        ...
        /// Pearson correlation of the best-matching key profile, roughly -1...1.
        let correlation: Float
    }

    /// Classifies a 12-element chroma vector into a key index with correlation.
    ///
    /// - Parameter chroma: A 12-element array of pitch class energies (C, C#, D, ..., B). Does not
    ...
    func classify(_ chroma: [Float]) -> Result {
        ...
        // A very low best correlation means noise, or material with no tonal content.
