...
/// Performs windowed FFT on audio frames and produces magnitude spectra. A Hann window is
/// applied before the FFT, matching the C++ CKey pipeline.
final class FFTProcessor: @unchecked Sendable {
    ...
    /// - Parameter blockLength: The number of samples per frame, rounded up internally to the
    ///   next power of two.
    ...
    func magnitudeSpectrum(of frame: [Float]) -> [Float] {
        // Zero-padded when the frame is shorter than the FFT.
        ...
        // vDSP's real FFT works on interleaved data packed into split complex form.
